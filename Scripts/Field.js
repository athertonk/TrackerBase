...
function StartDrawingField(sendr) {
  var okToCont = true;
  try {
...
function SelectField() { $("#uxFieldSelect0").click(); }
function EditField(sendr) {
  try {
    if (lmus.count > 0) {
      ...
      HiUser("No field exists.\n\nPlease create an area first.");
      return;
    }

  } catch (err) { HiUser(err, "Edit Field"); }
}
function EditField_Part2(sendr) {
  try {
    if (!editFeat || featureTypes.FIELD !== editingFeatType) { alert("Please select a field first."); return false; }
    infowindow.close();
    LoadFieldOptions("Edit");
    EditFeature(sendr, featureTypes.FIELD, editingFeatIndx, editingOid);
  } catch (err) { HiUser(err, "Edit Field 2"); }
}
function BeginNewField(sendr) {
  ...
  } catch (e) { HiUser(e, "Begin New Field"); }
}
...
function DeleteField(oid) {
  try {
    if (lmus.count > 0) {
      ...
      HiUser("No field exists to delete.");
      return;
    }
...
function DeleteField_Part2(oid) {
  try {
    ...
    if ("undefined" === typeof editingFeatType || featureTypes.FIELD !== editingFeatType
      /*|| "undefined" === typeof editingFeatIndx || 0 > editingFeatIndx*/ || "undefined" === typeof editingOid || 0 > editingOid) { alert("Please select the field first."); return false; }
    if ("undefined" === typeof oid || oid < 0) { alert("Please select the field first."); return false; }
    var ovToDel = lmus.GetFieldByOid(oid);

    var confMsg = CR + CR;
    confMsg += "Are you sure you want to delete your field?" + CR + CR + CR;
...
function ShowFieldTools(feattype, sendr) {
  try {
    inDrawMode = false;
