...
var featureTypes = {
  FIELD: 1, 
  properties: {
    1: { value: 1, name: "Field", clsName: "field", code: "F" },
  }
};
...
function StartDrawing(sendr) {
  ...
  try {
    switch (featureType) {
      case featureTypes.FIELD:
        okToCont = StartDrawingField(sendr);
        break;
      default:
        okToCont = false;
...
function SubmitToDatabase() {
  ...
  if (featureType === featureTypes.FIELD) {
    SubmitToDatabaseField();
  }
}
function SubmitToDatabaseField() {
  infowindow.close();
  ...
  if (actionType === actionTypes.ADD) {
    try {
      ...
      ClearFieldSelection();
      editingFeatIndx = -1;
      SetWebServiceIndicators(true, "Submitting Field");
      if ("Create" === action) {
        $.ajax({
    ...
    } catch (e) { HiUser(e, "Submit Field Add"); }
  }
    ...
    } catch (e) { HiUser(e, "Submit Field Edit"); }
  }
  StopDrawing();
}
...
function ShowFeatureTools(feattype, sendr) {
  try {
    if (featureType === featureTypes.FIELD) {
      ShowFieldTools(feattype, sendr); return;
