...
$(function () {
  ...
  if (emailCtl) emailCtl.href = "mailto:AthertonK@missouri.edu?subject=" + GetSiteSubdomain() + " Tracker (v" + GetSiteVersion() + ")";
...
function GetSiteSubdomain() {
  ...
  var subdomain = host.replace(".missouri.edu", "").replace("tracker", "");
  return subdomain;
}
...
function CallMenuClick(sendr) {
  ...
  if ("drawstuff" !== thisId) { return true; } //allow redirect

  if ("drawstuff" === thisId) { ProcessPrjHomeFlag(thisId); return false; } //just switch menus
...
function ProcessPrjHomeFlag(step) {
  ...
  if ("drawstuff" === step) {
    $(".draw-stuff").removeClass("accord-display-none");
...
function OpenForm(formName) {
  try {
    $("[id$=" + formName + "Container]").removeClass("display-none");
